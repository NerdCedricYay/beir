...
# Run reranker and save Results file
results = {}
with open("Results_all-MiniLM-L6-v2", "w", encoding="utf-8") as out:
    for qid, query_text in tqdm(queries, desc="Processing queries"):
        ranked = rerank(query_text, top_k=100, alpha=0.5) 
        results[qid] = {}
        for rank, (doc_id, score) in enumerate(ranked, start=1):
            out.write(f"{qid} Q0 {doc_id} {rank} {score:.6f} neural_all-MiniLM-L6-v2\n")
            results[qid][doc_id] = float(score)

# Evaluate MAP over all retrieved documents
_, map, _, _ = EvaluateRetrieval.evaluate(qrels, results, k_values=[100])

# Evaluate P@10
_, _, _, precision_10 = EvaluateRetrieval.evaluate(qrels, results, k_values=[10])

print(f"MAP: {map['MAP@100']:.4f}")
